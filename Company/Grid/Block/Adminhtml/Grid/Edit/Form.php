...
class Form extends \Magento\Backend\Block\Widget\Form\Generic
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset->addField(
            ...
            [
                ...
                'values'=>  [
                    ["value" => 'intrastate',"label" => __("Intrastate")],
                    ["value" => 'interstate',"label" => __("Interstate")],
                ]
            ]            
        );
        $fieldset->addField(
            ...
            [
                'name' => 'customer_tax',
                'label' => __('Is Customer Tax Exempted?'),
                'id' => 'customer_tax',
                'title' => __('Is Customer Tax Exempted?'),
                'values'=>  [
 ...
 //        'label'     => 'state',
 //        'required'  => false,
 //        'onchange' => 'print_city(wkgrid_city, this.selectedIndex)',
 //        'name'      => 'state',
 //        'values'    => ['--Please Select Statedf--']
 // ))->setAfterElementHtml("
...
var conceptName = jQuery('#wkgrid_country').find(':selected').val();
    if(conceptName==='IN')
    {
        jQuery('.field-city').show();
        jQuery('.field-company_city').hide();
    }else{
         jQuery('.field-city').hide();
        jQuery('.field-company_city').show();
        jQuery('#wkgrid_company_city').val('');
    }
                         ...
                         function print_city(city_id, city_index){
                            ...
                            option_str.options[0] = new Option('Select City','');
                             ...
                             for (var i=0; i<city_arr.length; i++) {
                                cityvalues = myTrim(city_arr[i]);
                                 
                              option_str.options[option_str.length] = new Option(cityvalues,cityvalues);
                             }
                        }
...
setTimeout(function(){
    ...
    console.log('ghhklhl');
     console.log(city);
     jQuery('#wkgrid_city option[value=city]').attr('selected', 'selected'); 
     jQuery('#wkgrid_city option').each(function (a, b) {

            jQuery('#wkgrid_city option[value=city]').attr('selected', 'selected'); 
        if (jQuery(this).text().toLowerCase() == city.toLowerCase() ){
                jQuery(this).attr('selected','selected');
                jQuery(this).trigger('change');
